...
This has only been tested on a mac and requires that the machine have `afplay` for playing audio. You can install it if you have [homebreew](https://brew.sh/) installed with the command `brew install afplay`. This code uses a system call to `afplay` to play the audio.
